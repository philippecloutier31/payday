...
const OUTPUT_FILE = 'usd_fee_report_v3.txt';
...
try {
    ...
    let output = '--- DETAILED MAINNET FEE AUDIT (USD) ---\n';
    output += 'ID | CRYPTO | AMOUNT | USD_VAL | FEE_USD\n';
    output += '---|--------|--------|---------|---------\n';

    sessions.forEach(s => {
        // ONLY Mainnet BTC and ETH
        if ((s.cryptocurrency === 'btc' || s.cryptocurrency === 'eth') &&
            (s.status === 'completed' || s.status === 'forwarded')) {
